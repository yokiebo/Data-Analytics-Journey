--SQL Practice: Join Operations 1
...
/* Q4. Use the same JOIN as in the previous question.
...
SELECT player, 
		...
		coach, 
		gtime
FROM game a JOIN goal b ON a.id = b.matchid JOIN eteam c ON b.teamid = c.id
WHERE gtime <= 10
...
FROM game a JOIN eteam c ON a.team1 = c.id
...
/* Q8. The example query shows all goals scored in the Germany-Greece quarterfinal.
...
SELECT teamname, 
		COUNT(*) AS goals
FROM goal b JOIN eteam c ON b.teamid = c.id
GROUP BY teamname
